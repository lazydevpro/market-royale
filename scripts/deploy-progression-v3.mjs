...
const reportPath = file("deployments/shannon-progression-v3.json");
const pendingPath = file(".testnet/progression-v3-pending.json");
const existingReport = fs.existsSync(reportPath)
  ? read("deployments/shannon-progression-v3.json")
  : null;
...
assert.equal(oldVersion, 2n, "Expected the active V2 sidecar");
assert.ok(
...
if (!report.completedSteps.includes("Fund V3 reserve"))
  assert.ok(
    ...
    "Deployer needs 192 tUSDC for the replacement protection reserve",
  );
await send("Approve V3 reserve", TOKEN, tokenAbi, "approve", [
  report.progression,
  RESERVE,
]);
await send("Fund V3 reserve", report.progression, artifact.abi, "fund", [
  RESERVE,
]);
await send(
  "Verify test players",
