// Deploys the V5 arena and its Shannon-only progression sidecar. Every signed
...
const reportPath = file("deployments/shannon-v5.json");
const pendingPath = file(".testnet/v5-deploy-pending.json");
const report = fs.existsSync(reportPath)
  ? read("deployments/shannon-v5.json")
  : {
      chainId: 50312,
      version: 5,
      host: host.address,
...
await deploy("Deploy V5 arena", "arena", arenaArtifact, [
  TOKEN,
  MODULE,
  CREATOR,
  VENUE,
...
await deploy("Deploy progression", "progression", progressionArtifact, [
  report.arena,
  TOKEN,
  host.address,
  firstSeasonEnds,
