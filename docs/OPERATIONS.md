...
2. As the event host, choose an entry contribution from 1–50 tUSDC and an equal starting vault from 5–250 tUSDC, along with minimum players, capacity, maximum rounds and market duration. The entry cannot exceed the vault. Scheduled starts align to a newly opened matching market, with up to 60 seconds of clock tolerance; an already-running window cannot be attached. The UI prefers a 15-minute window and only exposes five minutes if the trusted DreamDEX venue publishes that interval. The confirmed lobby displays the exact start time.
